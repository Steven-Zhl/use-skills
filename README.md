...
> My ideas — 一个 Claude Code **skill 市场（marketplace）**，打包三个 skills：

| Skill | 说明 |
|---|---|
| **as-doctor** | 疾病/症状问诊：结构化问诊 + 权威检索，提供分诊与就医建议（模型自动启用） |
| **find-information** | 非工作场景下浏览 AI Agent / 大模型 / 前端 / 国际局势领域的信息（模型自动启用） |
| **weekly-report** | 从 git 提交与未提交改动反推工作周报，照抄提交消息不如读 diff（模型自动启用） |
...
三个 skill 均为**模型自动调用**（非斜杠命令），命名空间为 `/use-skills:*`。
